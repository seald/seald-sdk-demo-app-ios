...
BOOL testSealdSsksTMR(const SealdCredentials* sealdCredentials)
{
    @try {
        ...
        SealdSsksTMRPlugin* ssksTMR2 = [[SealdSsksTMRPlugin alloc] initWithSsksURL:sealdCredentials->ssksURL appId:sealdCredentials->appId];
...
BOOL testSealdSDKWithCredentials(const SealdCredentials* sealdCredentials, const NSString* sealdDir)
{
    @try {
        ...
        NSDictionary<NSString*, SealdActionStatus*>* respRevoke = [es1SDK1 revokeRecipients:[NSArray arrayWithObject:user3AccountInfo.userId] error:&error];
        NSCAssert(error == nil, [error localizedDescription]);
        NSCAssert(respRevoke.count == 1, @"Unexpected response count.");
        NSCAssert(respRevoke[user3AccountInfo.userId].success == YES, @"Unexpected status.");
        ...
        NSCAssert(error == nil, [error localizedDescription]);
        NSCAssert(respOther.count == 2, @"Unexpected response count %lu", (unsigned long)respOther.count); // revoke user2 and group
        for (NSString* key in respOther) {
            NSCAssert(respOther[key].success == YES, @"Unexpected status.");
        }
        ...
        NSCAssert(error == nil, [error localizedDescription]);
        NSCAssert(respRevokeAll[user1AccountInfo.userId].success == YES, @"Unexpected status.");
